...
@ALGORITHMS.register('softmatch_fusion')
class SemiFusion(AlgorithmBase):
    """
    ...
    def train_step(self, x_lb, y_lb, t_lb, frax_lb, x_ulb_w, x_ulb_s, t_ulb, frax_ulb):
        ...
        if self.args.freeze_backbone:
            for param in self.model.named_parameters():
                if param[0].startswith('module.blocks'):
    ...
    def set_dataset(self):
        ...
        lb_dset, ulb_dset, eval_dset, mean, std = get_fracture(args=self.args, alg=self.algorithm, include_lb_to_ulb=True, target_type=self.args.target_type)
        test_dset = None
