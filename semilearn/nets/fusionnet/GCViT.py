...
from .gcn import GCN
class MulitDropoutGCN(nn.Module):
    def __init__(self, dropouts=[0.2,0.4,0.6,0.8], nfeat=1, nclass=64, nhid=[4,16]):
        self.gcns = nn.ModuleList([
            GCN(nfeat=nfeat, nclass=nclass, nhid=nhid, dropout=nn.Dropout(d))
            for d in dropouts
        ]) # 4channel*64nclass = 256embed_dim
    
    def forward(self, x, adj):
        embed = torch.cat([gcn(x, adj) for gcn in self.gcns], dim=-1)
        return torch.reshape(embed, (embed.shape[0],1,-1)) #B,patch_num,embed_dim
...
class GCViT(nn.Module):
    ...
    def forward(self, x, prompt_x, only_fc=False, only_feat=False, **kwargs):
        """
        Args:
