...
def get_sup_fracture_time(args, alg, include_lb_to_ulb=True, splitNum=5):
    ...
    get_meta = get_1198_meta(num_classes=args.num_classes, target_type=args.target_type)
    mean = torch.tensor(get_meta(info_data,mean=True)['prompt'][0])
    std = torch.tensor(get_meta(info_data,std=True)['prompt'][0])

    filter_lb = filter_1198_knowMOFtime(ban)
    filter_ulb = filter_1198_unknowMOFtime(ban)
    filter_all = filter_1198(ban)

    filter_list = [filter_all]
    NO_list = []
    suffix_list = []
    data_dict = [{'NO_list':[],'targets':[]} for _ in range(len(filter_list))] #targets好像没用上？忘了
    NO_Intersection = None # drop miss case
    ...
    return lb_dset, eval_dset, mean, std
        
class get_1198_meta(object):
    def __init__(self, num_classes, target_type):
        super().__init__()
    ...
    def __call__(self, df, NO=None, mean=False, std=False):
        ...
        gruop_prompt = [healthInfo[0:7],healthInfo[9:22],healthInfo[22:34],healthInfo[7:9]+healthInfo[34:45],BMD_T]
        ...
        regress_target = None
        if self.target_type=="ifMOF":
            target = classfity_target
        elif self.target_type=="fracTime":
            if ifMOF[0]==0 and fracTime[0]==90:
        ...
        if ifMOF[0]==0:
            ...
            censorship = [0]
        return {"prompt":[prompt], "target":target, "NO":NO, "censorship":censorship, "FRAXs":FRAXs}


class filter_1198_knowMOFtime(object):
    ...
    def filtrate(self, NO, df):
        ...
        if len(row)==0:
            #print("miss info: ",NO)
            return False
        if NO not in self.ban and row['是否MOF'].values[0]==1:
            return True
        elif NO not in self.ban and row['是否MOF'].values[0]==0 and row['any骨折时间月'].values[0]==90:
            return True
        else:
            return False

class filter_1198_unknowMOFtime(object):
    def __init__(self, ban=[]):
        self.ban = ban
    def filtrate(self, NO, df):
        """
        ...
        if len(row)==0:
            #print("miss info: ",NO)
            return False
        if NO not in self.ban and row['是否MOF'].values[0]==0 and row['any骨折时间月'].values[0]<90:
            return True
        else:
            return False
...
if __name__ == "__main__":
    ...
    dataset = BasicDataset(dataset_path=dataset_path,\
                    get_meta=get_1198_meta,\
                    info_data=info_data, rejecter=filter_1198_knowMOFtime(ban),\
                    image_pathlist=["1198/GRAYlspine", "1198/GRAYhip1"])
    dataset[0]
    print(len(dataset))
    split = spliter(dataset,rewrite_file=True)