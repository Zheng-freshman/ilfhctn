...
class AlgorithmBase:
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.loss_scaler = partial(GradScaler,device="cuda")()
        self.amp_cm = partial(autocast,device_type="cuda") if self.use_amp else contextlib.nullcontext
        self.gpu = args.gpu
        self.rank = args.rank
    ...
    def set_hooks(self):
        ...
        if self.args.use_aim:
            ...
            raise RuntimeError("No Aim package for Windows")
    ...
    def process_batch(self, input_args=None, **kwargs):
        ...
        for arg, var in kwargs.items():
            ...
            if isinstance(var, dict):
                ...
                var = [v.cuda(self.gpu) for v in var]
            else:
