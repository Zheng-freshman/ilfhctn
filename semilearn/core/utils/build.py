...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    """
    from semilearn.datasets import get_eurosat, get_medmnist, get_semi_aves, get_cifar, get_svhn, get_stl10, get_imagenet, get_json_dset, get_pkl_dset, get_fracture, get_fracture_time, get_sup_fracture_time, get_nsclc
    ...
    # custom dataset
        ...
        lb_dset, ulb_dset, eval_dset, test_dset = get_fracture(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
    elif dataset == 'fracture_time':
        lb_dset, ulb_dset, eval_dset, test_dset = get_fracture_time(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
    elif dataset == 'sup_fracture_time':
        lb_dset, ulb_dset, eval_dset, test_dset = get_sup_fracture_time(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
    elif dataset == 'nsclc':
        lb_dset, ulb_dset, eval_dset, test_dset = get_nsclc(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
    else:
        return None
...
def get_data_loader(args,
    ...
    if isinstance(data_sampler, str):
        ...
        return DataLoader(dset, batch_size=batch_size, shuffle=False, num_workers=num_workers, collate_fn=collact_fn,
                          pin_memory=pin_memory, sampler=data_sampler(weights=sampler_weights, dataset=dset, num_replicas=num_replicas, rank=rank, num_samples=num_samples),
