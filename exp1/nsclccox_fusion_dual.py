...
@ALGORITHMS.register('nsclccox_fusion_dual')
class NSCLCCoxFusionDual(AlgorithmBase):
    """
    ...
    def init(self, T, hard_label=True, dist_align=True, dist_uniform=True, ema_p=0.999, n_sigma=2, per_class=False):
        ...
        # align with GaussianNoise which expects list of tensors per time-branch
        self.t_weight = [torch.tensor([1 for _ in range(6)])]###<---之后可能改成显著性的倒数
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False):
        ...
        mse1 = np.mean((y_pred1 - y_true1)**2)
        mse2 = np.mean((y_pred2 - y_true2)**2)

        eval_dict = {'\n'+eval_dest+'/model_loss_1': total_loss[0] / total_num, 
                     eval_dest+'/model_loss_2': total_loss[1] / total_num,
                     '\n'+eval_dest+"/MAE_1": round(np.mean(errors1), 2), eval_dest+"/top-1-acc_1": round(accuracy1,2), eval_dest+"/MSE_1": round(mse1,2), eval_dest+"/C-index_1": round(cindex1,2), 
                     '\n'+eval_dest+"/MAE_2": round(np.mean(errors2), 2), eval_dest+"/top-1-acc_2": round(accuracy2,2), eval_dest+"/MSE_2": round(mse2,2), eval_dest+"/C-index_2": round(cindex2,2)}
        for i in range(len(y_info)):
            eval_dict[eval_dest+'/'+y_info[i]+'/true'] = y_true[i]
            eval_dict[eval_dest+'/'+y_info[i]+'/case1:'] = errors1[i]
            eval_dict[eval_dest+'/'+y_info[i]+'/case2:'] = errors2[i]
            eval_dict[eval_dest+'/'+y_info[i]+'/mse1'] = (y_true[i]-y_pred1[i])**2
            eval_dict[eval_dest+'/'+y_info[i]+'/mse2'] = (y_true[i]-y_pred2[i])**2
            eval_dict[eval_dest+'/'+y_info[i]+'/acc1'] = 100-100*(errors1[i]/y_true[i])
            eval_dict[eval_dest+'/'+y_info[i]+'/acc2'] = 100-100*(errors2[i]/y_true[i])
